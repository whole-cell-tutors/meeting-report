...
The primary challenge faced by the integration group was how to handle concurrent editing of shared state variables by multiple submodels. The integration group explored several potential strategies to manage concurrent writing. First, they explored sequentially simulating the submodels and updating the global state variables. This avoids the need for more complex strategies to merge variable changes. However, under this approach submodels are simulated with different variable values within each time step. Consequently, simulation predictions are sensitive to the submodel execution order. 
