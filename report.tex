...
%%% GOAL %%%
The set goal for the summer school was to re-build the Karr model using only standard representation formats and open software tools. 
This ambitious goal demanded much work ahead of the summer school itself:  
%%% TEAM BUILDING %%%
In preparation of the summer school the tutors decomposed the model into submodules. They assigned the students assigned to eight research groups, each led by a tutor and each \comment{responsable for one submodule}{Waltemath}{We could add the overview figure of the Karr model here, split into the eight submodules; or we could add the overview of all groups and participants here, but maybe this is a bit too unusual?}.  
During  meetings through Google hangout and using code sharing through GitHub,  students got used to the MATLAB whole-cell model and started to think about their specific sub-model(s), especially the interfaces. 
...
The Integration group developed an integration mechanism for the single submodules, using the software iBioSim~\cite{Stevens2013}. 
...
Other groups started by going through the MATLAB code and converting it into SBML. 
...
Each day closed with  a pleanry session where the single groups presented their results of the day, got feedback from the whole group, and further topics for discussion were planned.
Throughout the workshop, each student presented at least once.  
% How much time did students have to work?

% Special sessions
Additionally, we arranged for spontaneuous discussion rounds whenever problems occured in more than one team. 
...
\comment{tbw}{Waltemath}{I guess Chris is the one to write some more here? Please note that the issue is mentioned again further down in the discussions though} 
...
\comment{We were glad to have Dr. Karr present his model at the summer school.}{Jonathan, would you write this: Outline more specifically what students did learn, and how this was taught through the small group interactions, intro lectures, and popup seminars}
...
\comment{Elaborate background}{Waltemath}{Would like to ask Tom to help here, using the statistical data we have on background, degree of prior knowledge etc}
...
An overview of progress in the single modules is shown in \comment{Table~1}{Waltemath}{Are we going to put the table here? We could state that the data was gathered through  self-evaluation within the single teams}. 
...
The second reason for the delay in encoding is the lack of concepts to represent large state spaces sparsely. Currently, SBML enforced the enumeration of the entire space, whichi is inefficient for combinatorially large spaces. A first improvement is the introduction of arrays, as suggested in the specification of the SBML Arrays package~\cite{}. However, arrays alone will not solve this problem. 
...
\subsection{Interaction with the modeling community}
The positive effect on the standardization community became immediately visible:
\comment{Throughout the summer school, the participants discussed features of existing standards, thereby providing feedback on the usability of COMBINE standards and associated software tools.
This feedback is going to be forwarded into the COMBINE community, for example through presentations at the annual community meetings COMBINE and HARMONY.}{Waltemath}{Jonathan, I rewrote. Is your comment still valid: This is a bit confusing. Was this a course or a meeting? Who exactly was getting feedback -- students, tutors?}
...
\comment{Help}{Waltemath}{Need help here, not sure how to structure this part}
...
In addition, lessons learned from the summer school and weaknesses of COMBINE standards are scheduled for the 2015 standardization meetings HARMONY and COMBINE. 
