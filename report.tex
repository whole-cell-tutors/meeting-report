...
We organized the 2015 Whole-Cell Modeling Summer School to teach whole-cell modeling, as well as to evaluate the need for new modeling standards and tools by attempting to encode a recently published whole-cell model in SBML.
We describe several new standards, software tools, and databases which are needed to enable researchers to encode and simulate whole-cell models using SBML, including a graphical model editor, a multi-algorithm simulator, and several SGBN extensions.
Together these new standards and software tools could accelerate whole-cell modeling.
\end{abstract}
...
Here, we describe the summer school, outline our progress toward encoding the \textit{M. genitalium} model using SBML, and describe several standard extensions and software tools that are needed to support whole-cell modeling.
...
Another challenge to integrating the submodels was lack of simulator for the multi-algorithm model composition. The integration group plans to overcome this limitation by adding support for multi-algorithm to iBioSim.
...
\subsection{Progress}
The students produced preliminary SBML and SBGN versions of many of the \textit{M. genitalium} whole-cell model submodels. However, significant work remains to finish encoding the submodels, integrate the submodels into a single model, and test the combined model. Complete drafts are only available for a few of the simplest submodels. The current drafts of several submodels are also greatly simplified compared to their MATLAB versions. For example, the transcription and translation submodels drafts do not represent the polymerization of individual bases. In addition, none of the SBML-encoded submodels have been thoroughly tested or documented.
...
Prior to the school, SBML, SED-ML, and SBGN had never been used to represent whole-cell or similarly complex models. Consequently, not surprisingly, the summer school revealed several limitations of the existing simulation software tools and SBGN for large models. Importantly, the summer school facilitated discussions among modelers, tool developers, and model curators about how to best overcome these limitations.

\subsection{Current limitations}
As discussed above, the SBML simulators currently have limited support for model composition, particle-based state representation, arbitrary stochastic algorithms, and arrays. Consequently, whole-cell models can only be simulated from SBML by enumerating state spaces and arrays, formulating random algorithms as discrete stochastic simulations, and formulating each submodel using the same mathematical representation. However, for large models because this would result in infeasibly large XML files and millions of variables.

Furthermore, no SBML simulation software is currently capable of efficiently simulating models involving millions of variables. In addition, there is no user-friendly SBML editing tool for building models which use all of the \emph{comp}, \emph{multi}, \emph{arrays}, and \emph{fbc} packages. Currently, whole-cell model SBML files would have to be generated by scripts and then would not be easily editable.
...
Together, these software, database, and SBGN expansions would enable more researchers to more easily build, manage, simulate, and reproduce whole-cell models and simulations. These new standards and software tools would also enable researchers to build more comprehensive and more accurate models. Ultimately, these new standards and software would enable whole-cell modeling to support rational biological design and personalized medicine.
