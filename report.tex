...
\author{
	\IEEEauthorblockN{
        ...
        Pinar Pir\IEEEauthorrefmark{16} and\\
        2015 Whole-Cell Modeling Summer School Consortium\IEEEauthorrefmark{17} 
	}
    ...
    \IEEEauthorblockA{\IEEEauthorrefmark{17}Membership of the 2015 Whole-Cell Modeling Summer School Consortium is listed in Table~SI}
...
\IEEEtitleabstractindextext{
	...
	We found that standards would accelerate whole-cell modeling and that our current standards SBML, SED-ML, and SGBN must be expanded to support whole-cell modeling.
...
SED-ML is a standard language for describing computational experiments. SED-ML enables computational scientists to reproduce simulations by describing in detail the simulation setup, including setting the simulation algorithm and its parameters and tracking the parameter values used in each simulation. 
SBGN is a standard language for describing visual representations of models. \comment{SBGN also has restrictions regarding the visualization of whole-cell models.}{Karr}{Elaborate} 
Taken together, further work is needed to expand the capabilities of \comment{our}{Waltemath}{the} current systems biology standard languages
% SBML, CellML, SED-ML, and SBGN 
to support whole-cell modeling.
...
The first and foremost goal of the school was to train young researchers how to build whole-cell models, including how to develop sub-models of individual cellular pathways using different mathematical formalisms; how to assemble sub-models into a single model; and how to encode models using SBML, simulate models using SED-ML, visualize models using SBGN, and link model entities to semantic annotations in controlled vocabularies for systems biology \cite{courtot2011controlled}. 
The second goal of the school was to identify how the SBML, SED-ML, and SBGN languages must be expanded to support whole-cell modeling, as a representative of state-of-the-art modeling.
...
%\comment{
Following the summer school, %we surveyed students to evaluate the course.}{Karr}{What did the survey results show?} 
most of the %Almost all of 
...
In addition, we plan to submit them to open model repositories. 
...
Finally, the Volkswagen foundation provides funding for a follow-up event. 
That two-days meeting will focus on the complete and tested integration of a small number of sub-modules. 
